import{L as m,s as u,u as i,g as f,a as g}from"./utils-aQK5xiNv.js";function c(){const t=f("so-cart");return t?(Array.isArray(t)?t:[t]).filter(r=>r&&typeof r=="object"&&r.Id&&Array.isArray(r.Colors)&&r.Colors.length>0):[]}function l(t){return Number(t.FinalPrice)||0}function d(t){return Number(t.Quantity)||1}function p(t){const e=c();t<0||t>=e.length||(e.splice(t,1),u("so-cart",e),s(),i())}function y(t,e){const r=c();t<0||t>=r.length||(r[t].Quantity=e,u("so-cart",r),s(),i())}function _(){const t=document.querySelector(".product-list");t&&(t.addEventListener("click",e=>{const r=e.target.closest(".cart-card__remove");if(!r)return;const a=Number(r.dataset.index);Number.isInteger(a)&&p(a)}),t.addEventListener("change",e=>{const r=e.target.closest(".cart-card__quantity-input");if(!r)return;const a=Number(r.dataset.index),n=Number(r.value);Number.isInteger(a)&&n>0&&y(a,n)}))}function s(){const t=c(),e=document.querySelector(".product-list");e&&(t.length===0?e.innerHTML="":e.innerHTML=t.map((r,a)=>b(r,a)).join(""),C(t))}function C(t){const e=document.querySelector(".cart-footer"),r=document.querySelector(".cart-total");if(!e||!r)return;if(t.length===0){e.classList.add("hide"),r.textContent="Total: ";return}const a=t.reduce((n,o)=>n+l(o)*d(o),0);e.classList.remove("hide"),r.textContent=`Total: $${a.toFixed(2)}`}function b(t,e){var n,o;const r=((n=t.Colors[0])==null?void 0:n.ColorName)??"",a=t.Image||((o=t.Images)==null?void 0:o.PrimaryMedium);return`<li class="cart-card divider">
